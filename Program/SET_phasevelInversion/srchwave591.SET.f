...
c  maxnsta=300 -> maxnsta=500
      parameter (maxnfreq=1, maxnsta=500, maxpts=20000, nparam = 4000,
...
c     ranor : 4->8 test
c      integer*8 nxkern,nykern	
c      integer*8 nsta(maxevnts), iref(maxevnts),idnode(maxnodes)
c      integer*8 nfreq,nstapts(maxnsta),indx(nparam),nstacor,nobs
c      integer*8 istanum(maxevnts,maxnsta),istacor(maxnsta), 
c     1          nnodes, nevents, ntyp1                                
c      integer*8 idnum(maxevnts),ityp1sta(maxnsta)                               
c      integer*8 nevntsta(maxnsta),istavar(maxnsta)  
c      integer*8 jbox(maxevnts)
      ...
      character*2   nettemp
c      character*4   statemp
c     ranor : sta_name 4->5
      character*5   statemp
c     ranor : dummy,dummy1 60 -> 58  ! cutting length depends on the path
c      character*60  dummy, dummy1 
      character*59  dummy, dummy1  
...
c     ranor: similar with rdsetupsimul
c      character*4 staname(maxnsta)
      character*5 staname(maxnsta)
...
c        ranor:debug
c        读入每个事件每个台站记录所在的路径
c        write(*,'(a)') (fn(iev,i),i=1,nsta(iev))
      enddo
c      stop      
c     所有的文件读取都要由read(*,*) -> read(*,'(a)')
c     1
c     0.025
c     detail.040.591.080.SET
c     summar.040.591.080.SET
c     SETgridinp.dat
c     phampcor.040
c     covar.040.591.080.SET
c     mavamp.040.591.080.SET
c     tempd
c     SETrgnlBndryPts
c     stanumlist
c     3.40
c     10  0.25 0.25
c     sensspec040_3000.dat
c     velout.040.591.080.SET
c     gridphse040sNF
c     resolve.040.591.080.SET
c     gridphse.040.591.080.SET
      read(*,*) nfreq
...
c      read(*,*) fvelnodes
      ...
      read(*,'(a)') fresdiag
      read(*,'(a)') fendvel
c     ranor:debug 检查读取是否正常
c      write(*,*) nfreq
c      write(*,*) (freq(j), j= 1, nfreq)
c      write(*,'(a)') foutput,fsummary,finvrsnodes
c      write(*,'(a)') fftinput,fvariance,fmaxavamp,ftemp,fvelarea
c      write(*,'(a)') fstalist
c      write(*,*) unifvel,iterlimit,dampvel,dampaniso
c      write(*,'(a)') sensfn,fvelout,startvel,fresdiag,fendvel
c      stop
...
c       ranor: debug 对数据所在路径的字符串做了cut
c       所以statemp在cut之后读入
c          write(*,*) dummy1
c          write(*,*) statemp
	        rewind (13)
